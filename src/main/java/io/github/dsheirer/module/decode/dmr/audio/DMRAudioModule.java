...
import io.github.dsheirer.audio.exfiltrate.ZmqAudioExfiltrationService;
...
public class DMRAudioModule extends AmbeAudioModule implements IdentifierUpdateProvider, IMessageProvider
{
    ...
    private void produceAudio(byte[] frame, long timestamp)
    {
        try
        {
            ...
            float[] audioSamples = audioWithMetadata.getAudio();
            
            // Add audio to the normal audio segment processing
            addAudio(audioSamples);
            
            // Exfiltrate audio via ZMQ if enabled
            try
            {
                ZmqAudioExfiltrationService zmqService = ZmqAudioExfiltrationService.getInstance();
                if(zmqService.isEnabled())
                {
                    // Extract channel information for frequency
                    long frequency = extractFrequency();
                    
                    // Publish audio with metadata to ZMQ
                    zmqService.publishAudio(audioSamples, getIdentifierCollection(), "DMR", 
                                          getTimeslot(), timestamp, frequency);
                }
            }
            catch(Exception e)
            {
                // Don't let ZMQ errors affect normal audio processing
                mLog.debug("ZMQ audio exfiltration error: " + e.getMessage());
            }
    ...
    /**
     * Extracts frequency information from the identifier collection
     */
    private long extractFrequency()
    {
        // Try to get frequency from the current identifier collection
        // For now, return 0 as placeholder - frequency will be provided by the channel processor
        return 0L;
    }
